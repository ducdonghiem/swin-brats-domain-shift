...
class ModalityDecoder(nn.Module):  
    def __init__(self, in_channels=8, depth=155):
        super(ModalityDecoder, self).__init__()
        self.logits_conv = nn.Conv2d(in_channels, depth, kernel_size=1, padding=0, bias=True)
    
    def forward(self, x):
        logits = self.logits_conv(x)
        return logits  # (B, 155, 240, 240)
...
class ReconstructionBlock(nn.Module):
    def __init__(self, in_channels=48, hidden_channels=32, num_classes=4, original_depth=155):
        ...
        self.decoders = nn.ModuleList([
            ModalityDecoder(in_channels=hidden_channels // num_classes, depth=original_depth)
    ...
    def forward(self, x):
        ...
        block_size = self.hidden_channels // self.num_classes  # 32 // 4 = 8
        blocks = torch.split(x, block_size, dim=1)  # 4 tensors of (B, 155, 240, 240)
        
        # Process each block through its own decoder
        logits = []
        for block_idx, block in enumerate(blocks):
            logit = self.decoders[block_idx](block)  # (B, 155, 240, 240)
            logits.append(logit)
